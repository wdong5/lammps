      ...
      if (flag) {
        if (mlflag == 1){
         ...
         // Add the graph to the session
		  ...
		  auto input_tensor_mapped = input_tensor.tensor<double, 1>();
		  std::cout<<"created input_tensor_map\n";
		  input_tensor_mapped(0) = nbr_pj->d;
		  std::cout<< input_tensor_mapped(0)<<"\n";
		  input_tensor_mapped(1) = static_cast<double>(twbp->gamma);
		  std::cout<< input_tensor_mapped(1)<<"\n";
		  input_tensor_mapped(2) = static_cast<double>(twbp->D);
		  std::cout<< input_tensor_mapped(2)<<"\n";
		  input_tensor_mapped(3) = twbp->alpha;
		  input_tensor_mapped(4) = twbp->r_vdW;
		  std::cout<< input_tensor_mapped(4)<<"\n";
		  input_tensor_mapped(5) = twbp->lgcij;
		  input_tensor_mapped(6) = twbp->gamma_w;
		  std::cout<< input_tensor_mapped(6)<<"\n";
