      ...
      if (flag) {
        if (mlflag == 1){
         ...
         // Add the graph to the session
		  ...
		  input_tensor_mapped(0,1) = static_cast<float>(nbr_pjtwbp->gamma);
		  input_tensor_mapped(0,2) = static_cast<float>(nbr_pjtwbp->D);
		  input_tensor_mapped(0,3) = static_cast<float>(nbr_pjtwbp->alpha);
		  input_tensor_mapped(0,4) = static_cast<float>(nbr_pjtwbp->r_vdW);
		  input_tensor_mapped(0,5) = static_cast<float>(nbr_pjtwbp->lgcij);
		  input_tensor_mapped(0,6) = static_cast<float>(nbr_pjtwbp->gamma_w);
